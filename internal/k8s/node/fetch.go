	...
	err = k8sutil.Paginate(ctx, func(lo v1.ListOptions) (string, error) {
		...
		for idx := range list.Items {
			...
			nodes = append(nodes, &Info{
				...
				AllocatableEphemeralStorage: int64(node.Status.Allocatable.StorageEphemeral().AsApproximateFloat64()),
				AllocatableMemory:           node.Status.Allocatable.Memory().AsApproximateFloat64(),
				AllocatablePods:             int64(node.Status.Allocatable.Pods().AsApproximateFloat64()),
